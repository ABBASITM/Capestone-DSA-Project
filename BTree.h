...
public:
	...
	void splitChild(int placement, BTreeNode<T>* splitNode)
	{
		...
		{																// (order / 2) is min number of children
			newNode->keys[x] = splitNode->keys[minKeys + 1 + x];	// Supposed to be y->keys[(order / 2)] but since division is costly, I reused minKeys variable 
		...
		keys[placement] = splitNode->keys[minKeys];
	...
	void insertUnfilled(T data, string filePath, int lineNumber)
	{
		int i = noOfKeys - 1;
		...
		else
		{
			...
			children[i]->insertUnfilled(data, filePath, lineNumber);
		}
	}
	...
	int findKey(T data) // Find key that is greater or equal to data
	{
		int i = 0;

		while (i < noOfKeys && keys[i] < data)
			i++;
	...
	T prevKey(int i)
	{
		...
		return currentNode->keys[currentNode->noOfKeys - 1]; // return the last key of leaf (For key[0] number of keys is 1, so we use noOfKeys - 1)
	}
	...
	void takeFromLeftSibling(int i)
	{
		...
		keys[i - 1] = sibling->keys[sibling->noOfKeys - 1];
	...
	void takeFromRightSibling(int i)
	{
		...
		child->keys[child->noOfKeys] = keys[i];
		...
		{
			sibling->keys[x - 1] = sibling->keys[x];
	...
	void merge(int i)
	{
		...
		child->keys[minKeys] = keys[i]; // Code Check
		...
		{
			child->keys[minKeys + x + 1] = sibling->keys[x]; // Code Check
	...
	bool updateKeyInt(T data)
	{
		int i = findKey(data);

		if (i < noOfKeys && keys[i] == data)
		{
...
public:
	...
	void Insertion(T data, string filePath, int lineNumber)
	{
		if (root == nullptr)
		{
		...
		else
		{
			if (root->noOfKeys == order - 1)
			{
				...
				newNode->children[i]->insertUnfilled(data, filePath, lineNumber);
				root = newNode;
			}
			else
			{
				root->insertUnfilled(data, filePath, lineNumber);
			}
		}
	}
